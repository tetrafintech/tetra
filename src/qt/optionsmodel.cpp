...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TET);
    ...
    if (!settings.contains("nAnonymizeTetAmount"))
        settings.setValue("nAnonymizeTetAmount", 1000);

    nPrivateSendRounds = settings.value("nPrivateSendRounds").toLongLong();
    nAnonymizeTetAmount = settings.value("nAnonymizeTetAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTetAmount"))
        SoftSetArg("-anonymizetetamount", settings.value("nAnonymizeTetAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tet.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTetAmount:
            return QVariant(nAnonymizeTetAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeTetAmount:
            nAnonymizeTetAmount = value.toInt();
            settings.setValue("nAnonymizeTetAmount", nAnonymizeTetAmount);
            emit anonymizeTetAmountChanged(nAnonymizeTetAmount);
            break;
        case CoinControlFeatures:
