...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::TET)
    {
    }
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizeTetAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTetAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTetAmount = strAnonymizeTetAmount.remove(strAnonymizeTetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTetAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTetAmount * COIN) nMaxToAnonymize = nAnonymizeTetAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTetAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTetAmount));
        strAnonymizeTetAmount = strAnonymizeTetAmount.remove(strAnonymizeTetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTetAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
