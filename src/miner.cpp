...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TetMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TetMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TetMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tet-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running TetMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
